...
def set_checkout(request, return_url, cancel_url, error_url, template="paypal/setcheckout.html", currency="USD"):
    """
...
def do_refund(request, error_url, success_url, template="paypal/dorefund.html"):
    if request.POST:
        # normalize the given amount
        amount = request.POST.get("amount")
        ...
        # Now we are gone, redirect user to success page
        if request.user.is_authenticated():
            messages.info(request, _("Amount %s has been successfully refunded, "
                                     "your transaction id is '%s'" % (amount, response.trans_id)))
        return HttpResponseRedirect(success_url)
